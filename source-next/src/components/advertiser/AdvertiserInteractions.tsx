...
export function AdvertiserMediaCalculator() {
  ...
  const number = (value: number) => new Intl.NumberFormat('en-US', { maximumFractionDigits: value >= 1000 ? 0 : 2 }).format(value || 0);
  ...
  return (
    <div className="bradv-calc-shell">
      ...
      <div className="bradv-calc-results" aria-live="polite">
        ...
        <div className="bradv-calc-result-grid">
          <article><small>Impressions</small><strong>{number(forecast.impressions)}</strong><em>Budget ÷ CPM × 1,000</em></article>
          <article><small>Clicks</small><strong>{number(forecast.clicks)}</strong><em>Impressions × CTR</em></article>
          <article><small>Estimated CPC</small><strong>{money(forecast.cpc)}</strong><em>Spend ÷ clicks</em></article>
          <article><small>Conversions</small><strong>{number(forecast.conversions)}</strong><em>Clicks × conversion rate</em></article>
          <article><small>Estimated CPA</small><strong>{money(forecast.cpa)}</strong><em>Spend ÷ conversions</em></article>
          <article><small>Attributed revenue</small><strong>{money(forecast.revenue)}</strong><em>Conversions × revenue/conversion</em></article>
          <article className="is-roas"><small>Estimated ROAS</small><strong>{number(forecast.roas)}×</strong><em>Revenue ÷ spend</em></article>
        </div>
        <div className="bradv-goal-planner">
          ...
          <div className="bradv-goal-result"><small>Planning budget</small><strong>{money(goalBudget)}</strong><span>{number(targetConversions)} conversions × {money(targetCpa)} target CPA</span></div>
        </div>
      </div>
    </div>
  );
